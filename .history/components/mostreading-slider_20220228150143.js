...
const Sliders = () => {
    ...
    const [regionId, setRegionId] = useState(nu)

    const regionOshData = async () => {
      const respon = await fetch('http://api.kabar-media.kg/api/v1/articles/most-viewed/?region=7')
  ...
  return (
    ...
    <div style={{marginTop: '50px'}}>
      ...
      <div>
        {data.map((el) => {
          return(
            <button className={styles.btn} onClick={regionOshData} onFocus={() => }>{el.region}</button>
          )
        })}
      </div>
    </div>
