...
const Sliders = () => {
    ...
    const regionData = async () => {
      const respon = await fetch(`http://api.kabar-media.kg/api/v1/articles/most-viewed/?region=${regionId}`)
      const datas = await respon.json()
      setRes(datas.results)
      console.log(res)

      const respon2 = await fetch(`http://api.kabar-media.kg/api/v1/articles/interesting/?region=${regionId}`)
      const data2 = await respon2.json()
      setInterest(data2.results)
  ...
  return (
    ...
    <div style={{marginTop: '50px'}}>
      ...
      <div>
        {data.map((el) => {
          return(
            <button className={styles.btn} onClick={regionData} onFocus={() => setRegionId(el.id) }>{el.region}</button>
          )
        })}
      </div>
    </div>
    ...
    <div> 
      ...
      >
        {res.map((el) => (
            <div style={{marginTop: '20px', marginBottom:''}}>
                {el.media == null ? (
              ...
              <div style={{width: '250px', height: '170px', padding: '0 10px'}}>
                  <h4>{el.region}</h4>
                  <h3>{el.title}</h3>
              </div>
