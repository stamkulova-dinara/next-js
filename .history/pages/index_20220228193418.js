...
export default function Home({ news, categories, carouselSlide }) {
  ...
  const search =  async (e) => {
    ...
    const searchFetch = await fetch(`http://api.kabar-media.kg/api/v1/articles/search/?search=${inputValue}`)
    const result = await searchFetch.json()
    setInfoInCategory(result)
  }

  return (
    <div className={styles.container}>
      <div className={styles.search_blovk}>
      <input
